...
def convert_balltask_csv_to_bids(infile):
    ...
    df.participant = "sub-charms" + df.participant
    ...
    # put together bids tsv filename with zero-padded subject ID
    sub_id = "{:03d}".format(int(slider_outputs['id'][0]))
    outfile = f'data/sub-charms{sub_id}/sub-charms{sub_id}_ses-nf_task-{run_type}_run-{run_num:02d}.tsv'
