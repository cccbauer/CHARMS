...
then
	...
	--separator=" " \
	--add-combo="Step" --combo-values "setup_MGH|------LOCALIZER-----|2vol|resting_state|extract_rs_networks|process_roi_masks|register|------NEUROFEEDBACK------|2vol|register|feedback|cleanup|backup_reg_mni_masks_to_2vol" \
	--cancel-label "Exit" --ok-label "Run Selected Step"\
